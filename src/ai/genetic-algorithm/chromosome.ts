...
export class Chromosome {
    ...
    get winLossRatio(): number {
        return (this.numGamesLost + this.numGamesWon === 0) ? 0 : this.numGamesWon / (this.numGamesWon + this.numGamesLost);
    }

    constructor(
        public readonly weights: iChessAiHeuristicDataPoints<number>
    ) {
    ...
    public static makeOffspring(a: Chromosome, b: Chromosome): Chromosome {
        ...
        for (const key in a.weights) {
            // @ts-ignore
            mergedWeights[key] = this.mergeWeight(a.weights[key], b.weights[key]);
        }
